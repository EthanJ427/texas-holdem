...
(function () {
  ...
  window.__soundTests = async function () {
    ...
    suite('音色落在「不刺耳也不发闷」的区间内', () => {
      // 用一阶差分能量占比当亮度代理，两边都要卡住 —— 这两个上下界
      // 分别对应实际收到的两次反馈：太高是「呲呲」的齿音，太低是「像敲门」。
      // 0.005 那一版整体闷成一片，0.32~0.49 那一版扎耳朵。
      const cases = [
        ['发牌', card], ['筹码', chips], ['敲桌', knock],
        ['弃牌', fold], ['翻公共牌', flip], ['收底池', pot],
      ];
      for (const [name, r] of cases) {
        ok(r.brightness < 0.25, `${name}：亮度 ${r.brightness.toFixed(4)} < 0.25（不刺耳）`);
        ok(r.brightness > 0.012, `${name}：亮度 ${r.brightness.toFixed(4)} > 0.012（不发闷）`);
      }
    });
